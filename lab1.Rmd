...
### Basic data wrangling reminders

Data wrangling & summary statistics w/ `dplyr`

Find top 5 highest observations of trees by legal_status, do some wrangling, make a graph
...
blackwood_acacia <- sf_trees %>% 
  filter(stringr::str_detect(species, "Blackwood Acacia")) %>%
...
Helpful for combining or separating columns
```{r}
...
sf_trees_unite <- sf_trees %>% 
  tidyr::unite("id_status", tree_id:legal_status, 
               sep = "_cool!_")
```

### Make some actual maps of blackwood acacia trees in SF

`st_as_sf()` to convert lat/long to spatial coords
